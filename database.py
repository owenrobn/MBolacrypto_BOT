...
=======
import os
>>>>>>> 6e233e2 (Deploy: Render Worker (start.py) + enhanced_bot + DB_PATH + render.yaml)
import uuid
...
<<<<<<< HEAD
    def create_event(self, host_id: int, title: str, description: str = None) -> str:
=======
...
>>>>>>> 6e233e2 (Deploy: Render Worker (start.py) + enhanced_bot + DB_PATH + render.yaml)
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute('''
<<<<<<< HEAD
                INSERT INTO events (event_code, title, description, host_id)
                VALUES (?, ?, ?, ?)
            ''', (event_code, title, description, host_id))
=======
...
>>>>>>> 6e233e2 (Deploy: Render Worker (start.py) + enhanced_bot + DB_PATH + render.yaml)
    ...
    def get_event_by_code(self, event_code: str) -> Optional[dict]:
        """Get event information by event code."""
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute('''
<<<<<<< HEAD
                SELECT e.id, e.event_code, e.title, e.description, e.host_id, 
=======
                SELECT e.id, e.event_code, e.title, e.description, e.group_link, e.host_id, 
>>>>>>> 6e233e2 (Deploy: Render Worker (start.py) + enhanced_bot + DB_PATH + render.yaml)
                       e.created_at, e.is_active, u.first_name as host_name
                FROM events e
                JOIN users u ON e.host_id = u.user_id
                WHERE e.event_code = ? AND e.is_active = 1
            ''', (event_code,))
...
=======
    ...
    def update_event_group_link(self, event_id: int, group_link: str) -> bool:
        """Update the group link for an event."""
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute('''
                UPDATE events SET group_link = ? WHERE id = ?
            ''', (group_link, event_id))
            conn.commit()
...
>>>>>>> 6e233e2 (Deploy: Render Worker (start.py) + enhanced_bot + DB_PATH + render.yaml)
    def get_user_events(self, user_id: int) -> List[dict]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute('''
<<<<<<< HEAD
                SELECT id, event_code, title, description, created_at, is_active
=======
